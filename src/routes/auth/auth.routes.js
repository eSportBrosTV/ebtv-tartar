const express = require('express')

const { user } = require('../../schemas')
const authCtrl = require('../../controllers/auth/auth.controller')
const passport = require('passport')

const { validateBody, auth, authorize, loginLimiter } = require('../../middlewares')
const policies = require('../../policies')

const authRouter = express.Router()

authRouter.post("/login", loginLimiter, validateBody(user.login), passport.authenticate('local'), authCtrl.login)
authRouter.post("/logout", auth, authCtrl.logout)

authRouter.post("/register", auth, validateBody(user.create), authorize(policies.admin.isAdmin), authCtrl.register)

module.exports = authRouter